...
def _compute_pct() -> int:
    st = INDEX_STATUS
    if st["state"] in ("idle", "queued"):
        return 1 if st["state"] == "queued" else 0
...
def _run_rebuild():
    try:
        ...
        def cb(ev: str, pl: Dict):
            if ev == "scan_total":
                _set_status(
                    files_total=int(pl.get("files", 0)), files_done=0, message="Files discovered"
                )
...
def _retrieve(question: str, k: int, mode: str, use_rerank: bool):
    ...
    idx = get_index()
    if mode == "dense":
        raw = idx.query_dense(qv, k=max(k, RERANK_TOPN))
        meta_mode = "dense"
    elif mode == "sparse":
        raw = idx.query_sparse(question, k=max(k, RERANK_TOPN))
        meta_mode = "sparse"
    else:
        raw = idx.query_hybrid(qv, question, k=max(k, RERANK_TOPN), alpha=ALPHA)
        meta_mode = "hybrid"
...
def create_ticket(req: CreateTicketRequest):
    ...
    with conn:
        ...
        tid = cur.lastrowid
        row = conn.execute(
            "SELECT id,title,priority,status,created_at FROM tickets WHERE id=?", (tid,)
        ).fetchone()
    conn.close()
    return {
        "status": "ok",
...
async def ingest_file(file: UploadFile = File(...), title: Optional[str] = Form(None)):
    ...
    if ext in [".md", ".txt"]:
        ...
        bin_dir = pathlib.Path("data/raw/uploads")
        bin_dir.mkdir(parents=True, exist_ok=True)
        bin_path = bin_dir / (file.filename or "upload.bin")
