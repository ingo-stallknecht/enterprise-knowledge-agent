...
# ---- Secrets → Env (so existing helpers that read env keep working) ----
...
# HF caches (kept if Streamlit container stays warm; not persistent across image rebuilds)
...
# Minimal settings (reads configs/settings.yaml if present)
CFG = {
    **({"retrieval": {}}),
    **(load_cfg("configs/settings.yaml") if pathlib.Path("configs/settings.yaml").exists() else {})
}
...
def rebuild_index(progress) -> Dict:
    ...
    if texts:
        ...
        for s in range(0, len(texts), batch):
            ...
            progress.update(label=f"Embedding… {e}/{len(texts)} chunks", value=0.25 + 0.60*(e/max(1,len(texts))))
        X = np.vstack(vecs)
    else:
        X = np.zeros((0, 384), dtype="float32")

    idx = load_or_init_index()
    progress.update(label="Writing index…", value=0.90)
    idx.build(X, records)
...
def fmt_citations(pairs: List[Tuple[Dict,float]], k: int) -> List[dict]:
    ...
    for i,(r,sc) in enumerate(pairs[:k],1):
        ...
        cites.append({"rank": i, "source": r.get("source"), "score": float(sc),
                      "preview": (txt[:280]+"…") if len(txt)>280 else txt})
    return cites

# ----------------- UI -----------------
st.markdown("<div class='header-row'><div><h3>Enterprise Knowledge Agent (Streamlit-only)</h3><div class='small'>Runs fully inside Streamlit. Secrets-powered OpenAI. No external backend.</div></div><div><span class='badge badge-ok'>Online</span></div></div>", unsafe_allow_html=True)

tab_ask, tab_upload, tab_about = st.tabs(["Ask", "Upload", "About"])

with tab_ask:
    st.markdown("**Answers from your local corpus (GitLab curated + uploads). Shows citations and sentence→source mapping.**")
    ...
    mode = colA.selectbox("Retrieval mode", ["hybrid","dense","sparse"], index=["hybrid","dense","sparse"].index(RETRIEVAL_MODE))
    use_rr = colB.checkbox("Use reranker", value=USE_RERANKER, help="Disable for faster response on cold start.")
...
with tab_upload:
    st.markdown("**Upload `.md` or `.txt` — they will be added to the knowledge base.**")
...
- **Q&A**: extractive by default; GPT-assisted if `USE_OPENAI=true` in Streamlit Secrets
...
- **Note**: Streamlit Community Cloud storage is *ephemeral*. For long-term persistence, keep your corpus in Git or load from a URL on startup.
""")
