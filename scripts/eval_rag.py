...
def load_questions(path: str) -> List[Dict]:
    ...
    if not p.exists():
        return [
            {
                "q": "How are values applied in performance reviews?",
                "must_terms": ["values", "performance"],
                "relevant_sources": ["values.md"],
            }
        ]
...
def compute_metrics_for_query(
    ...
    for i, (rec, _sc) in enumerate(res_topk, start=1):
        ...
        is_rel = (must_terms and text_contains_any(txt, must_terms)) or (
            relevant_sources and source_matches(src, relevant_sources)
        )
    ...
    return {
        ...
        "precision_at_k": prec,
        "rr": rr,
        "first_rel_rank": first_rel_rank or 0,
...
def main():
    ...
    for q in questions:
        ...
        results.append(
            {
                "q": qtext,
                "must_terms": q.get("must_terms", []),
                "relevant_sources": q.get("relevant_sources", []),
                "metrics": metrics,
                "topk_sources": [normalize_src(r.get("source", "")) for r, _ in pairs[:K]],
            }
        )
